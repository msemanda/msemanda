...
export default function PharmacyDispensePage() {
    ...
    // Only used for legacy orders that reached this queue with no drug
    // priced yet (pre-existing orders from before pricing moved to
    // prescribing time). New orders already carry drugId/quantity/amount.
    ...
    // A prescription's price (and payment gate) is now set when the doctor
    // picks the drug from stock — ROUTINE orders wait for Finance to confirm
    // payment before they can be handed over; STAT/URGENT bypass the gate.
    ...
    const selectOrder = (order: RxOrder) => {
        ...
        setNotes("");
        if (!order.drugId) {
            // Legacy order with no priced drug yet — try to auto-match by name
            const match = drugs.find(d =>
                order.detail?.toLowerCase().includes(d.drugName.toLowerCase()) ||
                (d.genericName && order.detail?.toLowerCase().includes(d.genericName.toLowerCase()))
            );
            if (match) setSelectedDrug(match);
        }
    };

    // Legacy path only: this order reached the queue with no drug priced —
    // price it now and send the bill to Finance, same as prescribing would
    // have. The order then needs a second, separate Dispense action once paid.
    const handlePriceAndBill = async () => {
        if (!selected || !selectedDrug) return;
        const quantity = parseInt(qty) || 1;
        if (quantity > selectedDrug.quantity) return;
        setSaving(true);
        try {
            const amount = quantity * selectedDrug.unitPrice;
            await updateDoc(doc(db, "cpoeOrders", selected.id), {
                ...
                detail: `${selectedDrug.drugName} × ${quantity} ${selectedDrug.unit}`,
            });
            if (amount > 0) {
    ...
    const handleDispense = async () => {
        if (!selected || !selected.drugId || isGated(selected)) return;
        const drug = drugs.find(d => d.id === selected.drugId);
        if (!drug) return;
        const quantity = selected.quantity ?? 1;
        if (quantity > drug.quantity) return;
        ...
        try {
            ...
            // Deduct stock
            const newQty = drug.quantity - quantity;
            await updateDoc(doc(db, "pharmacyStock", drug.id), { quantity: newQty });
            ...
            // The bill already exists (created at prescribing time) — just
            // record that the drug has now actually been handed over.
            const billSnap = await getDocs(query(collection(db, "patientBills"), where("orderId", "==", selected.id)));
            if (billSnap.docs.length > 0) {
                await updateDoc(doc(db, "patientBills", billSnap.docs[0].id), {
                    dispensedAt: serverTimestamp(),
                    dispensedBy: profile?.name,
                });
            }

            setDrugs(prev => prev.map(d => d.id === drug.id ? { ...d, quantity: newQty } : d));
    ...
    const legacyQtyNum   = parseInt(qty) || 0;
    const legacyTotal    = selectedDrug ? legacyQtyNum * selectedDrug.unitPrice : 0;
    const legacyOverStock = selectedDrug ? legacyQtyNum > selectedDrug.quantity : false;
    const legacyCanBill  = !!selected && !selected.drugId && !!selectedDrug && !legacyOverStock && legacyQtyNum > 0 && !saving;

    const priced = !!selected?.drugId;
    const pricedDrug = priced ? drugs.find(d => d.id === selected!.drugId) : null;
    const pricedGated = selected ? isGated(selected) : false;
    const pricedOverStock = priced && pricedDrug ? (selected!.quantity ?? 1) > pricedDrug.quantity : false;
    const canDispense = priced && !pricedGated && !!pricedDrug && !pricedOverStock && !saving;

    const gatedCount = orders.filter(isGated).length;
        ...
        <div className="space-y-6 pb-10">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-0.5">Prescriptions are priced and billed when the doctor prescribes — payment must clear before handing over the drug</p>
                </div>
            ...
            {gatedCount > 0 && (
                <div className="flex items-center gap-3 p-4 bg-amber-50 rounded-2xl border border-amber-100">
                    ...
                    <p className="text-sm font-semibold text-amber-700">
                        {gatedCount} prescription{gatedCount !== 1 ? "s" : ""} awaiting payment confirmation before dispensing — STAT/URGENT orders are never held up.
                    </p>
                </div>
            )}
                ...
                <div className="lg:col-span-2 space-y-3">
                    ...
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-12 bg-white rounded-2xl border border-dashed border-gray-200 text-center">
                            ...
                            <p className="text-[10px] text-gray-400 mt-0.5">Orders appear here once a doctor prescribes medication.</p>
                        </div>
                    ) : (
                        <div className="space-y-2 max-h-[60vh] overflow-y-auto pr-1">
                            {orders.map(order => {
                                const active = selected?.id === order.id;
                                const gated = isGated(order);
                                return (
                        ...
                        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-5">
                            ...
                            <div className="p-4 bg-blue-50 rounded-xl border border-blue-100 space-y-2">
                                ...
                                <div className="pt-1 border-t border-blue-100">
                                    ...
                                    <p className="text-xs font-black text-blue-900">{selected.detail}</p>
                                </div>
                            </div>

                            {priced ? (
                                <>
                                    {/* Already priced at prescribing time — payment-gated */}
                                    {pricedGated ? (
                                        <div className="flex items-center gap-3 p-4 bg-amber-50 rounded-xl border border-amber-100">
                                            <Clock className="h-5 w-5 text-amber-600 shrink-0" />
                                            <p className="text-sm font-bold text-amber-700">
                                                Awaiting payment confirmation — UGX {selected.amount?.toLocaleString()} must be cleared by Finance before this can be dispensed.
                                            </p>
                                        </div>
                                    ) : pricedDrug ? (
                                        <div className="p-4 bg-green-50 rounded-xl border border-green-100">
                                            <p className="text-[10px] font-black text-green-500 uppercase tracking-widest mb-1">Ready to Dispense</p>
                                            <p className="text-sm font-black text-green-900">{pricedDrug.drugName}</p>
                                            <p className="text-xs text-green-700 mt-0.5">
                                                {selected.quantity} {selected.unit} · UGX {selected.amount?.toLocaleString()}
                                                {selected.paymentStatus === "PAID" ? " · Paid" : " · Proceeding (priority bypass)"}
                                            </p>
                                            {pricedOverStock && (
                                                <p className="text-[10px] text-red-600 font-bold mt-1 flex items-center gap-1">
                                                    <AlertTriangle className="h-3 w-3" /> Only {pricedDrug.quantity} {pricedDrug.unit} in stock — insufficient for this order
                                                </p>
                                            )}
                                        </div>
                                    ) : (
                                        <div className="flex items-center gap-3 p-4 bg-red-50 rounded-xl border border-red-100">
                                            ...
                                            <p className="text-sm font-bold text-red-700">The stock item originally selected is no longer in inventory.</p>
                                        </div>
                                    )}

                                    <div>
                                        <label className="text-[10px] font-black text-gray-500 uppercase tracking-wider mb-1.5 block">
                                            Dispensing Notes <span className="text-gray-300 font-normal normal-case">(handling notes)</span>
                                        </label>
                                        <textarea value={notes} onChange={e => setNotes(e.target.value)}
                                            rows={2}
                                            placeholder="e.g. Counselled patient on adherence."
                                ...
                                <>
                                    {/* Legacy order — no drug priced yet, price it and send the bill */}
                                    <div>
                                        <label className="text-[10px] font-black text-gray-500 uppercase tracking-wider mb-1.5 block">
                                        ...
                                        <div>
                                            ...
                                            {legacyOverStock ? (
                                                ...
                                                <p className="text-[10px] text-gray-500 mt-1">
                                                    Total: <span className="font-black text-gray-700">UGX {legacyTotal.toLocaleString()}</span>
                                                </p>
                                            ) : null}
                                        </div>
                                    )}
                                    ...
                                    <div className="flex gap-3">
                                        ...
                                        <button onClick={handlePriceAndBill} disabled={!legacyCanBill}
                                            className="flex-1 h-10 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold flex items-center justify-center gap-2 transition-colors disabled:opacity-40 shadow-sm">
                                            {saving
                                                ? <div className="animate-spin h-4 w-4 border-2 border-white/30 border-t-white rounded-full" />
                                                : <><CheckCircle2 className="h-4 w-4" /> Price &amp; Send Bill to Cashier</>}
                                        </button>
                                    </div>
                                </>
                            )}
