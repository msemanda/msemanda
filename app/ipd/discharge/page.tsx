...
import { LogOut, Search, RefreshCw, CheckCircle2, AlertCircle, ArrowRightLeft } from "lucide-react";
...
interface IpdAdmission {
    ...
    doctorName: string;
    diagnosis: string;
    status: "ADMITTED" | "DISCHARGED" | "REFERRED" | "TRANSFERRED";
    admittedAt?: any;
    referredTo?: string;
...
export default function IpdDischargePage() {
    ...
    const [tab, setTab] = useState<"admitted" | "discharged" | "referred">("admitted");
    const [referringId, setReferringId] = useState<string | null>(null);
    const [referredTo, setReferredTo] = useState("");
    const [referralReason, setReferralReason] = useState("");

    const STATUS_FOR_TAB: Record<typeof tab, string> = {
        admitted: "ADMITTED", discharged: "DISCHARGED", referred: "REFERRED",
    };
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const q = query(collection(db, "ipdAdmissions"), where("status", "==", STATUS_FOR_TAB[tab]));
    ...
    const handleRefer = async (admission: IpdAdmission) => {
        if (!referredTo.trim() || !referralReason.trim()) return;
        setDischarging(admission.id);
        try {
            await updateDoc(doc(db, "ipdAdmissions", admission.id), {
                status: "REFERRED",
            ...
            await notify({
                targetRole: "ADMIN",
                type:       "referral",
                title:      `IPD patient referred: ${admission.patientName}`,
    ...
    return (
        <div className="space-y-6 pb-10">
            ...
            <div className="flex gap-2">
                {(["admitted", "discharged", "referred"] as const).map(t => (
                    <button key={t} onClick={() => setTab(t)}
                        ...
                        {t === "admitted" ? "Currently Admitted" : t === "discharged" ? "Discharged" : "Referred Out"}
                    </button>
                ))}
            </div>
            ...
            {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-16 bg-white rounded-2xl border border-dashed border-gray-200 text-center">
                    ...
                    <p className="text-sm font-black text-gray-900 mb-1">
                        {tab === "admitted" ? "No admitted patients" : tab === "discharged" ? "No discharge records" : "No referral records"}
                    </p>
                    <p className="text-xs text-gray-400">
                        {tab === "admitted" ? "Admit patients from the Admissions page." : tab === "discharged" ? "Discharged patients will appear here." : "Patients referred to another facility will appear here."}
                    </p>
                </div>
            ) : (
                <div className="space-y-3">
                    <AnimatePresence>
                        {filtered.map((a, i) => {
                            ...
                            const isReferring = referringId === a.id;
                            return (
                                <motion.div key={a.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.97 }} transition={{ delay: i * 0.04 }}
                                    ...
                                    <div className="flex items-start justify-between gap-4 flex-wrap">
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            {tab === "admitted" && (
                                                <>
                                                    <button onClick={() => setReferringId(isReferring ? null : a.id)} disabled={!!discharging}
                                                        className="h-9 px-4 rounded-xl border border-orange-200 bg-orange-50 hover:bg-orange-100 text-orange-700 text-xs font-bold flex items-center gap-1.5 transition-colors disabled:opacity-50">
                                            ...
                                            {tab === "referred" && (
                                                <span className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-orange-50 text-orange-700 border border-orange-100 flex items-center gap-1">
                                                    <ArrowRightLeft className="h-3 w-3" /> Referred
                                                </span>
                                            )}
                                        </div>
                                    </div>
                                    <p className="text-xs text-gray-500 mt-3 pt-3 border-t border-gray-50">{a.diagnosis}</p>
                                    {tab === "referred" && a.referredTo && (
                                        <div className="mt-3 pt-3 border-t border-gray-50 text-xs text-gray-600">
                                            <span className="font-bold text-orange-700">Referred to {a.referredTo}</span> — {a.referralReason}
                                        </div>
                                    )}
                                    {isReferring && (
                                        <div className="mt-3 pt-3 border-t border-gray-50 flex flex-wrap items-end gap-3">
                                            <div>
                                                ...
                                                <input
                                                    ...
                                                    className="h-9 px-3 rounded-lg border border-gray-200 bg-gray-50 text-xs font-medium outline-none focus:border-orange-500 focus:bg-white w-56"
                                                />
                                            </div>
                                            <div>
                                                ...
                                                <input
                                                    ...
                                                    className="h-9 px-3 rounded-lg border border-gray-200 bg-gray-50 text-xs font-medium outline-none focus:border-orange-500 focus:bg-white w-64"
                                                />
                                            </div>
                                            <button
                                                onClick={() => handleRefer(a)}
                                                disabled={!!discharging || !referredTo.trim() || !referralReason.trim()}
                                                className="h-9 px-4 rounded-xl bg-orange-600 hover:bg-orange-700 disabled:opacity-50 text-white text-xs font-bold flex items-center gap-1.5 transition-colors"
                                            >
                                                {discharging === a.id
                                                    ? <div className="animate-spin h-3.5 w-3.5 border-2 border-white/30 border-t-white rounded-full" />
                                                    : <><ArrowRightLeft className="h-3.5 w-3.5" /> Confirm Referral</>}
                                            </button>
                                        </div>
                                    )}
                                </motion.div>
