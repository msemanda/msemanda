...
import { FlaskConical, Search, Clock, Loader2, RefreshCw, ArrowRight, AlertCircle } from "lucide-react";
...
export default function LabOrdersPage() {
    ...
    const isGated = (o: LabOrder) => o.paymentStatus === "UNPAID" && o.priority === "ROUTINE";
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-black text-gray-900 flex items-center gap-2">
                        <FlaskConical className="h-6 w-6 text-amber-600" /> Lab Test Orders
                    </h1>
                ...
                <button onClick={load} disabled={loading}
                    className="h-9 w-9 rounded-xl border border-gray-100 bg-white shadow-sm flex items-center justify-center text-gray-400 hover:text-amber-600 transition-colors disabled:opacity-50">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <div className="px-5 py-3.5 border-b border-gray-50 flex items-center gap-3 flex-wrap">
                    <div className="relative flex-1 min-w-[200px]">
                        ...
                        <input value={search} onChange={e => setSearch(e.target.value)}
                            className="w-full pl-9 pr-3 py-2 text-sm bg-gray-50 rounded-xl border-0 outline-none focus:ring-2 focus:ring-amber-500/20"
                            placeholder="Search patient or test…" />
                    </div>
                    <div className="flex gap-1.5 flex-wrap">
                        {FILTERS.map(f => (
                            <button key={f} onClick={() => setFilter(f)}
                                className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-colors ${filter === f ? "bg-amber-600 text-white" : "bg-gray-50 text-gray-500 hover:bg-gray-100"}`}>
                                {f.replace(/_/g, " ")}
                                {counts[f] > 0 && (
                ...
                {loading ? (
                    ...
                    <div className="py-20 text-center text-gray-400 text-sm">
                        {search || filter !== "ALL" ? "No orders match your search." : "No lab orders yet. Orders come from the doctor's CPOE module."}
                    </div>
                ) : (
                            ...
                            <motion.div key={o.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }}
                                ...
                                <div className="flex items-start justify-between gap-4">
                                    <div className="flex items-start gap-3 min-w-0">
                                        <div className="h-9 w-9 rounded-xl bg-amber-50 flex items-center justify-center text-amber-700 font-black text-sm shrink-0">
                                        ...
                                        <div className="min-w-0">
                                            ...
                                            <p className="text-xs text-gray-500 mt-0.5 truncate max-w-sm">{o.detail}</p>
                                            {o.notes && <p className="text-xs text-gray-400 truncate max-w-sm">{o.notes}</p>}
                                    ...
                                    <div className="flex items-center gap-2 shrink-0">
                                        ...
                                        {o.status === "PENDING" && isGated(o) ? (
                                            ...
                                            <button onClick={() => advance(o.id, "IN_PROGRESS")} disabled={updating === o.id}
                                                className="text-xs font-bold px-3 py-1.5 rounded-lg bg-amber-600 text-white hover:bg-amber-700 transition-colors disabled:opacity-50">
                                                {updating === o.id ? <Loader2 className="h-3 w-3 animate-spin inline" /> : "Start"}
                                            </button>
                                        )}
